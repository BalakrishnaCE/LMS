...
export default function Quiz({ quizReference, moduleId }: QuizProps) {
  ...
  const { data: progressDoc, isValidating: progressDocLoading } = useFrappeGetDoc('Quiz Progress', quizProgressId || '', { enabled: !!quizProgressId });
  ...
  // Handle showing instructions or completed results
  const handleShowInstructions = () => {
    // Check if we have existing progress that's completed
    if (progressDoc && progressDoc.score_added === 1) {
      setShowProgressOnly(true);
      setIsOpen(true);
    } else {
      // Show instructions for new attempts
  ...
  useEffect(() => {
    if (isOpen && quiz?.time_limit_mins && !isSubmitted && !showProgressOnly) {
  ...
  }, [isOpen, quiz?.time_limit_mins, isSubmitted, showProgressOnly]);
  ...
  return (
    <>
      <Button variant="outline" onClick={handleShowInstructions}>
        {progressDoc && progressDoc.score_added === 1 ? "View Quiz Results" : "Start Quiz"}
      </Button>
      
      {/* Instruction Dialog - Only show for new attempts */}
      <InstructionDialog
        open={showInstructions}
        onOpenChange={(open) => {
          // Don't allow opening instructions if we have completed progress
          if (open && progressDoc && progressDoc.score_added === 1) {
            return;
          }
