...
function groupByDepartment(modules: any[]): Record<string, any[]> {
    return modules.reduce<Record<string, any[]>>((acc, mod) => {
        const dept = mod.department?.trim() || "General";
...
export function LearnerModules({ itemsPerPage = 8 }: ModulesProps) {
    ...
    // Group sorted modules by department
        ...
        <div className="space-y-6">
            ...
            {!apiLoading && !dashboardLoading && !error && !dashboardError && sortedModules.length > 0 && (
                <div className="space-y-10 max-w-7xl mx-auto p-4">
                    {deptNames.map((dept) => {
                        const deptModules = groupedModules[dept];

                        return (
                            <div key={dept}>
                                {/* Department Header */}
                                <div className="flex items-center gap-2 mb-4">
                                    <div className="w-1 h-5 rounded-full bg-gradient-to-b from-primary to-emerald-500 shrink-0" />
                                    <h2 className="text-s font-bold text-foreground">
                                        {dept}
                                    </h2>
                                </div>
                                ...
                                >
                                    <AnimatePresence>
                                        {deptModules.map((module: any, idx: number) => {
