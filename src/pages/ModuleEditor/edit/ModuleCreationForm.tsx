...
export default function ModuleCreationForm() {
  ...
  const { data: existingModules } = useFrappeGetDocList("LMS Module", {
    fields: ["name1", "department", "assignment_based"],
    limit: 1000,
  });
  ...
  const handleCreateModule = async () => {
    ...
    const isDuplicate = existingModules?.some((mod: any) => {
      ...
      if (mod.name1?.trim().toLowerCase() !== moduleName.trim().toLowerCase()) {
        return false;
      }

      // If assignment bases are different, allow it
      if (mod.assignment_based !== assignmentBased) {
        return false;
      }
      
      // If they have the SAME assignment base, check department if applicable
      if (assignmentBased === "Department") {
        return mod.department === departmentSelected;
      }
